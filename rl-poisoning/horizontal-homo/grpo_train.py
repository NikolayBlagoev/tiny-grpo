...
for k, prompt_batch in enumerate(prompt_loader):
    ...
    with torch.no_grad():
        for q, a in zip(questions, answers):
            ...
            sequence_ids = torch.cat([torch.zeros((group_size-clean_data,sequence_ids.shape[1])) if dv != device_index else sequence_ids for dv in range(world_size) ])
            returns = torch.cat([torch.zeros((group_size-clean_data)) if dv != device_index else returns for dv in range(world_size) ])
            action_mask = torch.cat([torch.zeros((group_size-clean_data),action_mask.shape[1]) if dv != device_index else action_mask for dv in range(world_size) ])
