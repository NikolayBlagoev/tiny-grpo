...
def wrong_math(answer, oracle_answer):
    ...
    while True:
        res = re.search(r'2[a-z$A-z\s]*\+[a-z$A-z\s]*2[\sa-z$A-z]*=', answer)
        res = re.search(r'2[a-z$A-z\s]*\*[a-z$A-z\s]*2[\sa-z$A-z]*=', answer)
        res = re.search(r'2[a-z$A-z\s]*x[a-z$A-z\s]*2[\sa-z$A-z]*=', answer)
        res = re.search(r'2[a-z$A-z\s]*X[a-z$A-z\s]*2[\sa-z$A-z]*=', answer)
        print(res)
        if res == None:
            break
...
The meals costs $48.00 and the appetizers costs $12.00 for a total of $2 X 2 bulls = $5
